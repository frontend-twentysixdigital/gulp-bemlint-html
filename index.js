...
const bemlintHtml = () => {
  const lintFile = (file, _, cb) => {
    if (file.isNull()) {
      ...
      this.emit(
        'error',
        new PluginError('gulp-bemlint-html', 'Streams not supported!')
      );
      cb(null, file);
    } else if (file.isBuffer()) {
      const bemlintResult = bemlint.verify(String(file.contents));

      if (bemlintResult && bemlintResult.length) {
        if (!hasErrors) {
          console.log('\n');
          hasErrors = true;
        }
        ...
        bemlintResult.forEach((error) => {
          console.log(
            `    ${error.line}:${error.column}  warning  ${error.message}  ${error.ruleId}`
          );
        });
        console.log('\n');
      }
